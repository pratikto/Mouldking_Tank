...
/*****************************************
 * Library   : BDCmotor - Library for dual motor driver carrier for Arduino.
 ...
 * Source for BDCmotor
 *
 * Copyright (C) 2018  Joao Lopes https://github.com/JoaoLopesF/BDCmotor
 *
 ...
 *
 * This file contains the code for BDCmotor library.
 *
 */
...
void BDCmotor::attachMotor(uint8_t gpioIn1, uint8_t gpioIn2)
{
...
void BDCmotor::attachMotors(uint8_t gpioIn1, uint8_t gpioIn2, uint8_t gpioIn3, uint8_t gpioIn4)
{
	// debug
...
void BDCmotor::motorFullForward(uint8_t motor)
{
...
void BDCmotor::motorForward(uint8_t motor, uint8_t speed)
{
	if (!isMotorValid(motor)) {
		return;
	}
...
void BDCmotor::motorFullReverse(uint8_t motor)
{
	if (!isMotorValid(motor)) {
		return;
	}
...
void BDCmotor::motorReverse(uint8_t motor, uint8_t speed)
{
	if (!isMotorValid(motor)) {
		return;
	}
...
void BDCmotor::motorStop(uint8_t motor)
{
	if (!isMotorValid(motor)) {
		return;
	}
...
uint8_t BDCmotor::getMotorSpeed(uint8_t motor) {
...
boolean BDCmotor::isMotorForward(uint8_t motor) {
...
boolean BDCmotor::isMotorStopped(uint8_t motor) {
...
boolean BDCmotor::isMotorValid(uint8_t motor) {
